...
if not pGui then pGui = plr:WaitForChild("PlayerGui") end
...
function PotatoLib:CreateWindow(config)
    ...
    uisize.MinSize = Vector2.new(500, 50)
    uisize.Parent = mf
    ...
    function WindowObj:CreateTab(name)
        ...
        function TabObj:CreateToggle(text, callback)
            ...
            tBtn.MouseButton1Click:Connect(function()
                ...
                callback(state)
                WindowObj:Notify("Toggled", text .. " is now " .. tostring(state), 2)
            end)
        end
        ...
        function TabObj:CreateSlider(text, min, max, callback)
            ...
            local sBgS = Instance.new("UIStroke", sBg)
            sBgS.Color = c1
            sBgS.Thickness = 2
            sBgS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
            ...
            local sDragS = Instance.new("UIStroke", sDrag)
            sDragS.Color = c1
            sDragS.Thickness = 2
            sDragS.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
            ...
            uIS.InputEnded:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    if dragging then
                        ...
                        local pos = math.clamp((input.Position.X - sBg.AbsolutePosition.X) / sBg.AbsoluteSize.X, 0, 1)
                        local val = math.floor(min + ((max - min) * pos))
                        WindowObj:Notify("Slider Updated", text .. " set to " .. tostring(val), 1.5)
                    end
                end
            end)
